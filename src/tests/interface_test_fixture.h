...
class InterfaceTestFixture : public CircuitTestFixture {
 ...
 protected:
  ...
  virtual void SetUp() {
    ...
    if (tls1_ec) {
      BOOST_LOG_TRIVIAL(error)
          << "Fail resolving interface tls1 endpoint : " << tls1_ec.message();
    ...
    if (tls2_ec) {
      BOOST_LOG_TRIVIAL(error)
          << "Fail resolving interface tls2 endpoint : " << tls2_ec.message();
  ...
  void InterfaceOnline(const std::string& interface_id,
    ...
    if (!ec) {
      BOOST_LOG_TRIVIAL(info) << "Interface up : " << interface_id;
    }
